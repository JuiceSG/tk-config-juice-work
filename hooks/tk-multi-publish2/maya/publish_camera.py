...
class MayaCameraPublishPlugin(HookBaseClass):
        ...
        * ``description``: A description of the setting as a string.
        ...
        maya_camera_publish_settings = {
            ...
            "Format": {
                "type": "string",
                "default": None,
                "description": "Camera export file format"
            }
        }
    ...
    def accept(self, settings, item):
        ...
        * ``checked``: If ``True``, the created task will be checked in the UI,
        ...
        publish_format = settings["Format"].value
        ...
        if publish_template:
            publish_template_name = "publish_template_%s" % publish_format
            item.properties[publish_template_name] = publish_template
        ...
        if export_format is 'fbx':
            command_doesnt_exist = self._check_fbx_command()
            if command_doesnt_exist:
                return command_doesnt_exist
        # check that the AbcExport command is available!
        elif export_format is 'abc':
            command_doesnt_exist = self._check_abc_command()
            if command_doesnt_exist:
                return command_doesnt_exist

        # check if bakeToWorld script exist:
        command_doesnt_exist = self._check_bake_command()
        if command_doesnt_exist:
            return command_doesnt_exist
    ...
    def validate(self, settings, item):
        ...
        path = _session_path()
        publish_format = settings["Format"].value
        path_name = "path_%s" % publish_format
        publish_path_name = "publish_path_%s" % publish_format
        ...
        work_template = item.parent.properties.get("work_template")
        publish_template_name = "publish_template_%s" % publish_format
        publish_template = item.properties.get(publish_template_name)
        ...
        item.properties[publish_path_name] = publish_path
    ...
    def publish(self, settings, item):
        ...
        # dodaje prawidlowy patch publishera dla instancji
        publish_path = item.properties[publish_path_name]
        item.properties["path"] = publish_path
        item.properties[publish_path_name] = publish_path

        self.logger.debug('PUBLISHER------------ ' + publish_path)
        # ensure the publish folder exists:
        publish_folder = os.path.dirname(publish_path)
        self.parent.ensure_folder_exists(publish_folder)

        # execute suitable exporter
        # and set the publish type in the item's properties. the base plugin will
        # use this when registering the file with Shotgun
        self.logger.debug("Publish format: %s" % publish_format)
        if publish_format == 'abc':
            try:
                self._export_abc(publish_path, cam_name)
            except Exception as e:
                self.logger.error("Failed to export ABC camera: %s" % e)
            item.properties["publish_type"] = "ABC Camera"
        elif publish_format == 'fbx':
            try:
                self._export_fbx(publish_path)
            except Exception as e:
                self.logger.error("Failed to export FBX camera: %s" % e)
    ...
    def _export_abc(self, publish_path,cam_name):
        self.logger.debug("Exporting ABC camera")
        path = '%s' % (publish_path.replace(os.path.sep, "/"))
        self.logger.debug(path)
        cmds.duplicate(inputConnections=True)  # duplikuje cam_shape
        tmp_select = cmds.ls(selection=True)  # wylapie i przechowa nazwe obiektu zduplikowanego
        tmp_select = tmp_select[0]
        self.logger.debug(tmp_select)
        mel.eval('bakeToWorld')
        cmds.select(tmp_select)
        first_frame = cmds.findKeyframe(which='first')
        last_frame = cmds.findKeyframe(which='last')
        command = '-frameRange %s %s -dataFormat ogawa -root %s -file %s' % (int(first_frame), int(last_frame), tmp_select ,path)
        cmds.AbcExport(j=command)
        cmds.delete(tmp_select)
        self.logger.debug('DONE')
        return True

    def _export_fbx(self, publish_path):
        self.logger.debug("Exporting FBX camera")
        fbx_export_cmd = 'FBXExport -f "%s" -s' % (publish_path.replace(os.path.sep, "/"),)
        cmds.duplicate(inputConnections=True)
        tmp_select = cmds.ls(selection=True)
        mel.eval('bakeToWorld')
        cmds.select(tmp_select)
    ...
    def _check_abc_command(self):
        if not mel.eval("exists \"AbcExport\""):
            self.logger.debug(
                "Item not accepted because abc export command 'AbcExport'"
                "is not available. Perhaps the plugin is not enabled?"
            )
            return {"accepted": False}
        return None

    def _check_bake_command(self):
        if not mel.eval("exists \"bakeToWorld\""):
            self.logger.debug(
                "Item not accepted because baking command 'bakeToWorld'"
                "Ensure you have good .env file with corrects shelves"
            )
